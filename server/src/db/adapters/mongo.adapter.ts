...
export class MongoAdapter implements IStoreXRepository {
  public readonly driverName = 'mongodb';
  ...
  constructor() {
    ...
    if (uri && !uri.includes('your-mongodb')) {
      ...
      console.log('[StoreX DB] MongoAdapter configured with URI:', uri);
    } else {
      console.warn('[StoreX DB] MongoAdapter: Missing MONGODB_URI in .env. Falling back to local memory store.');
    }
  }
